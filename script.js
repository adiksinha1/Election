...
const sectionIds = ['overview', 'timeline', 'assistant', 'faq'];
...
const setActiveStep = (index) => {
  ...
  document.querySelectorAll('.steps-card').forEach((el, cardIndex) => {
    el.classList.toggle('active', cardIndex === index);
  });

  miniSteps.forEach((step, stepIndex) => {
    step.classList.toggle('active', stepIndex === index);
  });

  focusTitle.textContent = selected.title;
  focusText.textContent = selected.detail;
  insightTitle.textContent = selected.title;
  insightText.textContent = selected.detail;
};
...
const renderTimeline = (dateValue) => {
  ...
  timelineFill.style.transform = 'scaleX(0.18)';
...
clearChat.addEventListener('click', () => {
  ...
  toggleChatEmptyState();
  addMessage('assistant', 'Assistant', 'Chat cleared. Ask anything about registration, voting, timelines, or results.');
});
